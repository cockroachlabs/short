...
var contentOverrides = map[string]string{
...
func (s *slurp) run() error {
	...
	fmt.Fprintf(out, "// Assets to be served.\n")
	fmt.Fprintf(out, "var Assets = map[string]struct{ContentType string; Data[] byte;} {\n")
	for _, inc := range include {
		data, err := ioutil.ReadFile(inc)
		if err != nil {
			return errors.Wrapf(err, "reading file %s", inc)
		}
		typ := contentOverrides[filepath.Ext(inc)]
		if typ == "" {
			typ = http.DetectContentType(data)
		}
		rel, _ := filepath.Rel(s.directory, inc)
		fmt.Fprintf(out, "%q: {%q, []byte{", rel, typ)
